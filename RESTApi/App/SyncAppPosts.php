 ...
 */
class SyncAppPosts extends BaseAPI
{
	/**
	...
	{
		// complete post type slug regix
		return '/app/(?P<app_id>[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})/posts/(?P<post_type>[a-zA-Z0-9_-]+)';
	}
	...
	public function execute(WP_REST_Request $request): WP_REST_Response
	{
		...
		AppPost::create_bulk_relation($app_id, $post_type);
